...
def _repair_link_references():
    ...
    Scoped to non-Shams AI Gateway DocTypes because SAG does not ship any
    ...
    sag_doctypes = frappe.get_all("DocType", filters={"module": "Shams AI Gateway"}, pluck="name")
...
def _recover_stranded_hrms_records():
    ...
    SAG's autoname is format:SK-{#####}, so any tabFAC Skill row whose name
    ...
    FACSkill = frappe.qb.DocType("SAG Skill")
    ...
    frappe.qb.from_(FACSkill).delete().where(not_sag_autoname).run()
