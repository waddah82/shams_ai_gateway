...
@frappe.whitelist(methods=["POST"])
def sync_skill_markdown_files() -> dict:
    """Generate or update Markdown files for every published SAG Skill."""
    frappe.only_for(["System Manager", "Assistant Admin"])
    try:
        from shams_ai_gateway.utils.skill_file_manager import sync_published_skill_files

        result = sync_published_skill_files()
        frappe.db.commit()
        result["message"] = (
            f"Synchronized {result['count']} skill file(s): "
        ...
        frappe.db.rollback()
        frappe.log_error(
            title="SAG Skill Markdown Synchronization Failed",
