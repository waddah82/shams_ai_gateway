...
from shams_ai_gateway.shams_ai_gateway.server import assistantServer
...
class AssistantCoreSettings(Document):
    ...
    def restart_shams_ai_gateway(self):
    ...
    def on_update(self):
        ...
        from shams_ai_gateway.shams_ai_gateway.server import get_server_instance
        ...
        if self.server_enabled:
            if server_was_enabled or not server.running:
                ...
                frappe.enqueue(
                    "shams_ai_gateway.shams_ai_gateway.server.enable_background_api", queue="short"
                )
        else:
...
def get_context(context):
    ...
    context.settings = frappe.get_doc("Shams AI Gateway Settings")
