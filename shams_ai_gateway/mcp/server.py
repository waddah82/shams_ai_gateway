...
class MCPServer:
    ...
    def handle(self, request: Request, response: Response, tool_registry: Optional[Dict] = None) -> Response:
        ...
        frappe.logger("sag_mcp_requests", allow_site=True).info(
            {
                "method": method,
    ...
    def _handle_tools_list(self, params: Dict, tool_registry: Optional[Dict] = None) -> Dict:
        ...
        # Load admin-authored descriptions once for the whole tools/list
        # response. They supply the main text; in replace mode the linked
        # skill URI is still retained as detailed guidance.
        configured_descriptions = {}
        try:
            tool_names = [tool.get("name") for tool in tool_registry.values() if tool.get("name")]
            if tool_names and frappe.db.table_exists("SAG Tool Configuration"):
                rows = frappe.get_all(
                ...
                configured_descriptions = {
                    row["tool_name"]: row["description"].strip()
                    for row in rows
                    if row.get("tool_name") and (row.get("description") or "").strip()
                }
        except Exception as exc:
            frappe.logger().warning(f"Failed to load configured MCP tool descriptions: {exc}")
        ...
        for tool in tool_registry.values():
            ...
            # In replace mode, retain the linked skill reference. When an
            # admin-authored DocType description exists it remains the main
            # description, and only the guidance URI is appended.
            if skill_replace_map and tool["name"] in skill_replace_map:
                skill_info = skill_replace_map[tool["name"]]
                skill_uri = f"fac://skills/{skill_info['skill_id']}"
                configured_description = configured_descriptions.get(tool["name"])
                if configured_description:
                    description = configured_description
                    if skill_uri not in description:
                        description = f"{description.rstrip()} Detailed guidance: {skill_uri}"
                else:
                    description = (
                        f"{tool['name']}: {skill_info['description']}. "
                        f"Detailed guidance: {skill_uri}"
                    )
            else:
                # Outside replace mode, the DocType description still wins;
                # the Python class description is the fallback.
                description = configured_descriptions.get(tool["name"], description)
