...
This patch migrates the enabled_plugins_list JSON array from SAG Settings
to individual SAG Plugin Configuration records for atomic operations and proper caching.

Before: SAG Settings.enabled_plugins_list = '["core", "visualization"]'
...
def execute():
    ...
    frappe.reload_doc("sag", "doctype", "sag_plugin_configuration")

    # 1. Read existing JSON from SAG Settings
    enabled_list = []
    try:
