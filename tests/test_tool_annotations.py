...
class TestToolsListConfiguredDescriptions(BaseAssistantTest):
    """SAG Tool Configuration descriptions are authoritative in tools/list."""

    def test_configured_description_wins_even_in_skill_replace_mode(self):
        from shams_ai_gateway.mcp.server import MCPServer
        ...
        settings = MagicMock(skill_mode="replace")
        skill_manager = MagicMock()
        skill_manager.get_tool_skill_map.return_value = {
            "create_document": {
                "description": "Description from skill",
                "skill_id": "create-document-usage",
            }
        }

        with ExitStack() as stack:
            stack.enter_context(patch.object(frappe.db, "table_exists", return_value=True))
            stack.enter_context(
                patch.object(
                    frappe,
                    "get_all",
                    return_value=[
                        {
                            ...
                            "description": "Description edited in SAG Tool Configuration",
                        }
                    ],
                )
            )
            stack.enter_context(patch.object(frappe, "get_single", return_value=settings))
            stack.enter_context(
                patch(
                    "shams_ai_gateway.api.handlers.resources.get_skill_manager",
        ...
        self.assertEqual(
            ...
            "Description edited in SAG Tool Configuration",
        )

    def test_python_description_is_fallback_for_blank_configuration(self):
        from shams_ai_gateway.mcp.server import MCPServer
        ...
        settings = MagicMock(skill_mode="supplementary")
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch.object(
                    frappe,
                    "get_all",
                    return_value=[{"tool_name": "get_document", "description": "   "}],
                )
            )
            stack.enter_context(patch.object(frappe, "get_single", return_value=settings))
        ...
        self.assertEqual(result["tools"][0]["description"], "Description from Python")
