...
def after_install():
    ...
    # Set default values for Shams AI Gateway Settings
...
def _install_system_skills():
    ...
    try:
        ...
        existing_system_skills = frappe.get_all(
            "SAG Skill", filters={"is_system": 1}, fields=["name", "skill_id", "source_app"]
        )

        deleted_count = 0
        for existing in existing_system_skills:
            # Skills owned by another app are managed by _install_app_skills().
            # Never remove them merely because they are absent from SAG's own
            # system_skills.json manifest.
            if existing.source_app not in (None, "", "shams_ai_gateway"):
                continue
...
def _set_settings_defaults():
    """Set default values for Shams AI Gateway Settings on fresh install.
    ...
    if default_values:
        try:
            doc = frappe.get_doc("Shams AI Gateway Settings")
            for fieldname, value in default_values:
                doc.set(fieldname, value)
            doc.flags.ignore_mandatory = True
            doc.save(ignore_permissions=True)
            frappe.logger("migration_hooks").info("Set default values for Shams AI Gateway Settings")
